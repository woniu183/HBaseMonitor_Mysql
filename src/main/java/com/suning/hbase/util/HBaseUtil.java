...
public class HBaseUtil {
  ...
  public static Long getConfRegionMax(){
	  Long rsize  =(long) 0;
	try {
		HBaseAdmin admin = new HBaseAdmin(configuration);
		  Configuration conf = admin.getConfiguration();
		  String regionmax = conf.get("hbase.hregion.max.filesize");	 
		  rsize = Long.parseLong(regionmax.substring(0, regionmax.length()-9));
		  admin.close();
      ...
      for (ServerName rs : rss) {
        ...
        for (RegionLoad region : load.getRegionsLoad().values()) {
         ...
         if(tableParamMap.containsKey(TableParam.totalRequestCount)){     
            totalRequestCount =region.getRequestsCount()+ Long.parseLong(tableParamMap.get(TableParam.totalRequestCount));
         }else{
          ...
          if(tableParamMap.containsKey(TableParam.writeRequestCount)){
            tableParamMap.put( TableParam.writeRequestCount,String.valueOf(region.getWriteRequestsCount()+ Long.parseLong(tableParamMap.get(TableParam.writeRequestCount))));
          }else{
          ...
          if(tableParamMap.containsKey(TableParam.readRequestCount)){
            tableParamMap.put( TableParam.readRequestCount,String.valueOf(region.getReadRequestsCount()+ Long.parseLong(tableParamMap.get(TableParam.readRequestCount))));
          }else{
          ...
          if(tableParamMap.containsKey(TableParam.storefileSizeMB)){
            tableParamMap.put( TableParam.storefileSizeMB,String.valueOf(region.getStorefileSizeMB()+ Long.parseLong(tableParamMap.get(TableParam.storefileSizeMB))));
          }else{
          ...
          if(tableParamMap.containsKey(TableParam.MemStoreSizeMB)){
            tableParamMap.put( TableParam.MemStoreSizeMB,String.valueOf(region.getMemStoreSizeMB()+ Long.parseLong(tableParamMap.get(TableParam.MemStoreSizeMB))));
            if(Long.parseLong(tableParamMap.get(TableParam.MemStoreSizeMB))>0){
          ...
          if(tableParamMap.containsKey(TableParam.regionNum)){
            tableParamMap.put( TableParam.regionNum,String.valueOf(1 + Long.parseLong(tableParamMap.get(TableParam.regionNum))));
          }else{
